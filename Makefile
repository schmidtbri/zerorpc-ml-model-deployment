...
clean-build:  ## clean build artifacts
	...
	rm -rf dist
	rm -rf vendors
	rm -rf model_zerorpc_service.egg-info
...
test-reports: clean-pyc clean-test ## Run unit test suite with reporting
	...
	mkdir ./reports/badge
	python -m coverage run --source model_zerorpc_service -m pytest --verbose --color=yes --html=./reports/unit_tests/report.html --junitxml=./reports/unit_tests/report.xml $(TEST_PATH)
...
check-docstyle:  ## checks the style of the docstrings against PEP257
	pydocstyle model_zerorpc_service

check-security:  ## checks for common security vulnerabilities
	bandit -r model_zerorpc_service
